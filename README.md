...
[할일]
판매자 페이지, 판매자 페이지오 api 문서화 하기(위키), 테스트코드
아래는 문서화 할때 참고할 내용
판매자 페이지(마이페이지는 내정보이고, 주문을 만든후에는 내 주문이 있고, 이와 별개로 판매자 페이지는 
판매자가 올린 상품리스트 라고 생각하면 된다.)
판매자 페이지는 프론트단에서 마이페이지의 url과 판매자 페이지 url을 잘 구분할것을 주의하라.
/seller로 시작한다.
마이페이지에서 auth가 MEMBER라면 나의 주문페이지가 뜨고
SELLER라면 판매자 페이지가 뜨게 하면된다.
